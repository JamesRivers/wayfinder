...
Create a repeatable AWX bootstrap flow backed by a git mirror of the Alloy bundle on the host. The bootstrap script `scripts/bootstrap-awx-alloy.py` now creates or updates the `observability` organization, the `mon` project, the `alloy-inventory` inventory, and the `alloy-template` job template in AWX. The project source is `git://172.16.47.163:9418/alloy-template-bundle.git`, and re-running the bootstrap script is the supported refresh path for future bundle updates. Verification was performed by syncing the project successfully and confirming the SCM revision `c48122714d53b1984612fbcd3aa74bce82e45428`.
